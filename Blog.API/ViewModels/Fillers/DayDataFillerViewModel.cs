...
namespace Blog.API.ViewModels.Fillers
{
    public class DayDataFillerViewModel : BaseDoWorkViewModel
    {
        ...
        {
            //以上证指数的数据为基准
            var ie = await base.GetStockRealTimeFormNetEase(new List<string>() { Utils.Constants.SHIndexId });
        ...
        private DateTime ParseFromStream(Stream stream)
        {
            ...
            using (StreamReader reader = new StreamReader(stream, Encoding.UTF8))
            {
                ...
                var span = doc.DocumentNode.SelectSingleNode("//span[@class='refresh_time']");



                string dateString = span.InnerText.Trim();
